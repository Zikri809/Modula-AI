...
export default async function (files: File[], uid: string) {
    ...
    try {
        ...
        if (!gemini_response || !gemini_response.candidates)
            throw new Error('gemini_response or the candidates missing');
        ...
        for (
            let index = 0;
            index < gemini_response_json.response.length;
            index++
        ) {
            //calculate the token used by extracted content
            const token_response = await gemini.models.countTokens({
                model: 'gemini-2.0-flash',
                contents:
                    gemini_response_json.response[index]
                        .mark_down_extracted_content,
            });

            file_meta_data[index].token_count = token_response.totalTokens ?? 0;
            file_meta_data[index].extracted_text =
                gemini_response_json.response[
                    index
                ].mark_down_extracted_content;
