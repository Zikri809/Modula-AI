...
const params_schema = z.object({
    ...
    confidence_level: z
        .number()
        .nonnegative()
        .min(0, 'value of the confidence level must be >= 0')
        .optional(),
    storage_ref: z.string().optional(),
    token_count: z.number().nonnegative().min(0).optional(),
    gemini_uri_part: z
        .object({
            fileData: z.object({
                mimeType: z.string(),
...
export default async function (
    chat_id: string,
    message_id: string,
    insert_params: z.infer<typeof array_schema>
) {
    if (!chat_id || !message_id)
        throw new Error(
            'chat_id and message_id must be provided must be provided'
        );
    ...
    const final = data.map((element) => {
        return {
            ...
            gemini_expiration_time: new Date(
                Date.now() + 48 * 3600 * 1000
            ).toISOString(),
        };
    });
    try {
        const { error } = await supabase.from('file_meta_data').insert(final);
