...
export default async function (chat_id: string, llm_model: string) {
    try {
        ...
        const { data, error } = await supabse
            .from('file_meta_data')
            .select(
                'storage_ref,gemini_uri_part, gemini_expiration_time,file_id'
            )
            .eq('chat_id', chat_id);
        ...
        for (const element of data) {
            ...
            if (
                Date.now() > new Date(element.gemini_expiration_time).getTime()
            ) {
                ...
                const downloaded_blob = await download_from_storage(
                    element.storage_ref
                );
