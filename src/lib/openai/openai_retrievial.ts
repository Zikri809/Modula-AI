...
export default async function (chat_id: string, text_only: boolean) {
    try {
        ...
        const { data, error } = await supabse
            .from('file_meta_data')
            .select(
                'storage_ref,file_name,extracted_text,token_count,confidence_level,file_id'
            )
            .eq('chat_id', chat_id);
        ...
        for (const {
            ...
            if (confidence_level < lowest_confidence)
                lowest_confidence = confidence_level;
            total_file_token += token_count;
            if (extracted_text && token_count < 20000) {
                content.push({
                    text: `${decoded_file_name} :${extracted_text}`,
                    type: 'text',
                } as ChatCompletionContentPart);
            }
        }

        retrieved_extracted_file.push({
            role: 'user',
            content: [...content],
