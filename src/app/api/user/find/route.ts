...
export  async function GET(request: NextRequest){
    ...
    try{
        ...
        if(!user_data.exists) return NextResponse.json({message: ' The doc with the uid does not exist in the db',uid:uid},{status:404})
        if(!req_type){
        ...
        return NextResponse.json({message: "Error trying to retrieve user data",error: String(error), uid:uid},{status:500})
    }
