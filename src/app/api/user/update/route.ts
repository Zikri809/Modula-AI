...
export async function PATCH(request: NextRequest) {
    ...
    try{
        const db_update = await update_user(uid as string, json_body, user_details_operation)
        if(db_update) return NextResponse.json({message: 'field updated successfully.'}, {status: 200})
