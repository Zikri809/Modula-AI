...
export async function PATCH(request: NextRequest){
    ...
    if(!chat_id) return NextResponse.json({error: "no chat_id"}, {status:400})
    if(!chat_title) return NextResponse.json({error: "no chat_title"}, {status:400})


    try{
        const result_db = await updateChat(chat_id, chat_title)
        if(!result_db) return NextResponse.json({message: 'error updating chat'}, {status:500})
        return NextResponse.json({message: 'successfully updated'},{status:200})
    }
    catch(error){
        console.error(error)
        return NextResponse.json({message:"error updating the chat title ", affected: chat_id,error:String(error)})
    }
}