...
export async function POST(request: NextRequest){
    ...
    if(!chat_id) return NextResponse.json({message:'please provide chat_id in request params'}, {status:400});
    ...
    try{
        ...
        if(db) return NextResponse.json({message: 'user messages retrieved succesfully ',response: db},{status:200});
